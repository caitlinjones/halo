...
#' @param regionNum      current region number
#' @param annotationNum  current annotation number
...
#' @param HasEndcaps     [???]
#' @param NegativeROA    [???]
#' @param v              verbose; print extra status messages; default=FALSE
#' @param lf             log file; default=NULL
#' @param debug          logical indicating whether to print debug statements
#' @return nothing
parseAnnotationXML <- function(node, fovName=NULL, outFile=NULL, 
  ...
  goTime <- function(node, fovName=NULL, outFile=NULL, 
                        LineColor="", Name="", Visible="", RegionType="", HasEndcaps=0,
                        NegativeROA=0, v=FALSE, lf=NULL, debug=FALSE){
    ...
    if(xmlSize(node) > 0){
       for(i in 1:xmlSize(node)){
           goTime(node[[i]], fovName=fovName, LineColor=LineColor, 
                       Name=Name, Visible=Visible, RegionType=RegionType, 
                       HasEndcaps=HasEndcaps, NegativeROA=NegativeROA,
                       outFile=outFile, v=v, lf=lf, debug=debug)
       }
    }
  }

  res <- goTime(node, fovName=fovName, outFile=outFile, 
           LineColor=LineColor, Name=Name, Visible=Visible, RegionType=RegionType, 
           HasEndcaps=HasEndcaps, NegativeROA=NegativeROA, v=v, lf=lf, debug=debug)  
...
#' @export
readHaloAnnotations <- function(annoteFile, v=TRUE, logFile=NULL, debug=FALSE){
...
subDivideLongSegments<-function(boundary,maxSegLength) {
    ...
    bline=boundary %>% select(X,Y) %>% mutate(SegNo=seq(nrow(.)))
    newPts=list()
...
#
clipToBounding<-function(x,bb){
    stop("spatialV3:168:SHOULD USE trimBoundaryToBB")
    xx=x %>%
    filter(X>=bb[1] & X<=bb[3]) %>%
    filter(Y>=bb[2] & Y<=bb[4]) %>%
    select(X,Y)
    #
    # Add an pair of NA's to last row to
    # break links between interface regions
    xx=rbind(xx,c(NA,NA))
    xx
}

clipToBoundingNoSelect<-function(x,bb){
    stop("spatialV3:181:SHOULD USE trimBoundaryToBB")
    xx=x %>%
    filter(X>=bb[1] & X<=bb[3]) %>%
    filter(Y>=bb[2] & Y<=bb[4])
    xx
