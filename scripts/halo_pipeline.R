...
################################################
allTbls <- NULL
if(!is.null(pp$marker_combo_table_file) && !updatedExclusions){
    print(paste0("Loading marker combo table from file: ",pp$marker_combo_table_file))
    wb <- loadWorkbook(pp$marker_combo_table_file)
    sheets <- getSheets(wb)
    ...
    xlsxFile <- file.path(pp$study_dir, paste0("marker_combo_table_",format(Sys.time(), "%Y%m%d"),".xlsx"))
    saveWorkbook(mcw, xlsxFile)

    pp$marker_combo_table_file <- xlsxFile
