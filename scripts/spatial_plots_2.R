...
if(args$plotDensityByBand){
    ...
    if(is.null(pp$plot_config_file)){
        stop("Can not plot infiltration density. Plot config required.")
    }
    cat(paste0("getting density for ",length(markers)," marker combinations...\n"))
    den <- getMarkerDensityTable(markers, dat=dat, dataFiles=pp$data_files, annotationsDirs=pp$annotations_dirs, outDir=pp$out_dir,
                             areaFiles=pp$area_files, densityFiles=pp$density_files, sampleOrder=pp$sample_order,
                             writeCSVfiles=writeCSVfiles, pad=pad, byBand=byBand, maxDistanceFromInterface=maxDistanceFromInterface,
                             bandWidth=bandWidth, markerSetName=markerSetName, sortByMarker=sortByMarker, funcMarker=funcMarker,
                             maxG=maxG, calcFOVarea=calcFOVarea)
